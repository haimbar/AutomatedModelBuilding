...
M <- dat[,-1]
colnames(M)[1] <- "y"
if(takelog)         # can log-transform the data, but with QR it's not necessary
  M$y <- log(1+M$y) # since we don't assume normal errors.
M0 <- M
set.seed(114322)
...
pdf("Figures/O3.pdf", width=5, height=5)
...
summCoef <- summary(res$qremFit[[2]]$fitted.mod)$coef